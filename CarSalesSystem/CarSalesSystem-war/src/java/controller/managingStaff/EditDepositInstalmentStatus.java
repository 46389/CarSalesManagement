 ...
 * @author Chew Jin Ni
 */
public class EditDepositInstalmentStatus extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String type = request.getParameter("status_type");
        String status = request.getParameter("status");
        
        try (PrintWriter out = response.getWriter()) {
            if("deposit".equals(type)){
                Deposit dp = DepositFacade.find(id);
                dp.setStatus(Deposit.Status.valueOf(status));
                DepositFacade.edit(dp);
                
            }else if("instalment".equals(type)){
                InstalmentPlan ip = InstalmentPlanFacade.find(id);
                ip.setPlan_status(InstalmentPlan.PlanStatus.valueOf(status));
                InstalmentPlanFacade.edit(ip);
            }
            response.sendRedirect(request.getContextPath() + "/FetchDepositInstalment");
        }catch(Exception e){
            System.out.println("Error in editing deposit or instalment status: " + e);
        }
    }
