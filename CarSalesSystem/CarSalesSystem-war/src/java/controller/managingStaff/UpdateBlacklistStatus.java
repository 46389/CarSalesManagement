 ...
 * @author Chew Jin Ni
 */
public class UpdateBlacklistStatus extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            out.println(blacklist_id);
            out.println(request_status);
            
            Blacklists blacklists = BlacklistsFacade.find(blacklist_id);
            blacklists.setStatus(Blacklists.Status.valueOf(request_status));
            BlacklistsFacade.edit(blacklists);
            
            response.sendRedirect(request.getContextPath() + "/FetchBlacklistCustomers");
        }catch(Exception e){
            System.out.println("Error in updating blacklist status: " + e);
        }
    }
