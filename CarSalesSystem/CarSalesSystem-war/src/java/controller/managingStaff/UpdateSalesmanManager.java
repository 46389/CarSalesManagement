 ...
 * @author Chew Jin Ni
 */
public class UpdateSalesmanManager extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String salesman_id = request.getParameter("selected_salesman");
            String manager_id = request.getParameter("selected_manager");
            
            Salesman salesman = SalesmanFacade.find(salesman_id);
            ManagingStaff manager = ManagingStaffFacade.find(manager_id);
            salesman.setManagingStaff(manager);
            SalesmanFacade.edit(salesman);
            
            response.sendRedirect(request.getContextPath() + "/FetchSalesmanAccount");
        }catch(Exception e){
            System.out.println("Error in updating salesman manager: " + e);
        }
    }
