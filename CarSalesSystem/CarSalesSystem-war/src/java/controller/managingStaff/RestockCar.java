 ...
 * @author Chew Jin Ni
 */
public class RestockCar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int stock = Integer.parseInt(request.getParameter("restock_number"));
        
        try (PrintWriter out = response.getWriter()) {
            CarType cartype = CarTypeFacade.find(car_id);
            for(int i = 1; i <= stock; i++){
                CarStock carstock = new CarStock(cartype);
                CarStockFacade.create(carstock);
            }
            
            response.sendRedirect(request.getContextPath() + "/FetchCarInformation");
        }catch(Exception e){
            System.out.println("Error in restocking car: " + e);
        }
    }
